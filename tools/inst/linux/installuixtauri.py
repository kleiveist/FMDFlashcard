...
def detect_linux_family() -> Tuple[str, Dict[str, str]]:
    ...
    if os_id in {"debian", "ubuntu"} or "debian" in like or "ubuntu" in like:
        return "debian", osr
    if os_id in {"arch", "manjaro", "endeavouros", "cachyos"} or "arch" in like:
        return "arch", osr
...
def install_build_deps(family: str) -> None:
    if family == "debian":
        _install_apt(["build-essential", "curl", "wget", "file", "pkg-config"])
        return
    if family == "arch":
        _install_pacman(["base-devel", "curl", "wget", "file", "pkgconf"])
        return
    raise RuntimeError(f"Unsupported Linux family for auto-install: {family}")
...
def install_node_deps(family: str) -> None:
    if family == "debian":
        _install_apt(["nodejs", "npm"])
        return
    if family == "arch":
        _install_pacman(["nodejs", "npm", "pnpm"])
        return
    raise RuntimeError(f"Unsupported Linux family for auto-install: {family}")
...
def ensure_pnpm(family: str) -> None:
    if which("pnpm"):
        print(f"{ICONS['ok']} pnpm gefunden.")
        return
    if _DRY_RUN:
        print(f"{ICONS['info']} pnpm fehlt; dry-run wuerde installieren.")
        return

    # Arch: prefer pacman (avoids npm -g permission/EACCES).
    if family == "arch" and which("pacman"):
        try:
            _install_pacman(["pnpm"])
        except Exception:
            pass
        if which("pnpm"):
            print(f"{ICONS['ok']} pnpm installiert (pacman).")
            return
    ...
    if which("corepack"):
        ...
        if which("pnpm"):
            print(f"{ICONS['ok']} pnpm aktiviert (corepack).")
            return

    # Fallback: npm -g (may require sudo).
    if which("npm"):
        try:
            run(["npm", "i", "-g", "pnpm"], check=True)
        except Exception:
            run(["sudo", "npm", "i", "-g", "pnpm"], check=True)
        if which("pnpm"):
            print(f"{ICONS['ok']} pnpm installiert (npm -g).")
            return
...
def ensure_rust(family: str) -> None:
    if rust_ready():
        print(f"{ICONS['ok']} Rust Toolchain aktiv.")
        return
    if _DRY_RUN:
        print(f"{ICONS['info']} Rust fehlt/inaktiv; dry-run wuerde rustup+stable installieren.")
        return

    if which("rustup") is None:
        try:
            install_rustup_pkg(family)
        except Exception:
            pass
    ...
    if not rust_ready():
        raise RuntimeError(
            "Rust installed but rustc/cargo still not usable. Open a new shell and re-run."
        )
...
def report_rust_status() -> None:
    ...
    if which("rustup"):
        ...
        if p.returncode == 0:
            print(f"{ICONS['info']} rustup show:\n{p.stdout.strip()}")
    print(
        f"{ICONS['ok']} Rust bereit."
        if rust_ready()
        else f"{ICONS['warn']} Rust noch nicht bereit."
    )


def ensure_target_dir(target_dir: Path, force: bool) -> bool:
    if target_dir.exists():
        if force or (target_dir.is_dir() and not any(target_dir.iterdir())):
            return True
        print(f"{ICONS['warn']} Target exists and is not empty: {target_dir}")
        print(
            f"{ICONS['info']} Skipping scaffold; continuing with install steps in that directory."
        )
        return False
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        description="Prepare Linux for Tauri (WASD libs, pnpm, rustup, scaffold)."
    )
