...
/**
 * @file frontend/src/components/PreviewPanel.interactions.test.ts
 *
 * Zweck:
...
describe("PreviewPanel edit-safe interactions", () => {
  ...
  it("keeps database blocks stable after entering and leaving markdown edit mode", async () => {
    const markdown = [
      "Intro",
      "",
      "::::",
    ...
    const initialDatabaseBlock = container.querySelector(
      ".preview-markdown-view-block-database-block .database-block",
    );
    expect(initialDatabaseBlock).toBeTruthy();
  ...
  it("promotes heading syntax typed into a blank block after leaving the line", async () => {
    ...
    act(() => {
      ...
      blankLine.textContent = "# Überschrift";
      editable?.dispatchEvent(new Event("input", { bubbles: true, cancelable: true }));
    });
    await act(async () => {
      await Promise.resolve();
  ...
  it("removes quote markers from hash-prefixed lines in markdown view edit flow", () => {
    const { container, cleanup: localCleanup } = buildHarness(
      ["> #helpHinweis", "> # Überschrift"].join("\n"),
    );
    cleanup = localCleanup;
