...
The parser looks at every line between `#exam` and `#endexam`. A new task begins when a trimmed line starts with a number from **1 to 20**. The following variations are all valid:
...
3. `#endexam`, which ensures the final task is emitted.
