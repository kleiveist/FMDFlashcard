...
| Module | Entry point | Caller | Inputs / env | Outputs / side effects | Edit when |
| --- | --- | --- | --- | --- | --- |
| [`tools/inst/run.py`](../../../tools/inst/run.py) | `run_install(dry_run=False)` | `control.py --run` and `--start` | `DISPLAY`, `WAYLAND_DISPLAY`, `frontend/package.json`, `node_modules`, `pnpm`, Rust tools. | May run `pnpm install`; starts `pnpm tauri dev`; may try `xvfb-run` on headless Linux. | Local desktop startup behavior changes. |
