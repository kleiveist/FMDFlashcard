...
const SHAPES: CanvasNodeShape[] = ["rounded-rectangle", "rectangle", "ellipse", "diamond"];
...
const snapValue = (value: number, gridSize: number, snapEnabled: boolean) =>
  snapEnabled && gridSize > 0 ? Math.round(value / gridSize) * gridSize : Math.round(value);
...
const resolveAnchorPoint = (node: CanvasNode, side: CanvasSide | undefined): CanvasPoint => {
...
const resolveBoundaryAnchor = (
  ...
  const horizontalT =
    Math.abs(deltaX) < 0.001 ? Number.POSITIVE_INFINITY : halfWidth / Math.abs(deltaX);
  const verticalT =
    Math.abs(deltaY) < 0.001 ? Number.POSITIVE_INFINITY : halfHeight / Math.abs(deltaY);
...
const buildEdgeRenderRow = (
  ...
  const { fromPoint, toPoint, fromSide, toSide } = resolveAutoEdgeAnchors(fromNode, toNode);
...
const resolveViewport = (document: CanvasDocument): CanvasViewport => {
  ...
  const x =
    typeof viewport.x === "number" && Number.isFinite(viewport.x) ? viewport.x : DEFAULT_VIEWPORT.x;
  const y =
    typeof viewport.y === "number" && Number.isFinite(viewport.y) ? viewport.y : DEFAULT_VIEWPORT.y;
  const zoom =
  ...
  (
    ...
    useLayoutEffect(() => {
      ...
      const updateContextMenuPosition = () => {
        ...
        if (
          menuHeight > 0 &&
          top + menuHeight > window.innerHeight - CONTEXT_MENU_VIEWPORT_MARGIN
        ) {
          top = Math.max(
            CONTEXT_MENU_VIEWPORT_MARGIN,
    ...
    const saveCustomColorSlot = useCallback(
      (slot: CanvasCustomColorSlot) => {
        const nextSlots = normalizeCanvasCustomColorSlots(
          normalizedCustomColors.map((current) => (current.slot === slot.slot ? slot : current)),
        );
    ...
    const switchMode = useCallback(
      ...
      [codeDraft, decorateForPersistence, loadValidationError, mode, persistDocument],
    );
    ...
    const requestDeleteNodeIds = useCallback(
      (nodeIds: string[]) => {
        ...
        const nodes = nodeIds
          .map((nodeId) => nodesById.get(nodeId))
          .filter((node): node is CanvasNode => Boolean(node));
        const requiresConfirmation = nodeIds.length > 1 || nodes.some((node) => isGroupNode(node));
    ...
    }, [applyDraftChange, mode, requestDeleteNodeIds, selectedEdgeId, selectedNodeIds]);
    ...
    const startConnectionDrag = useCallback(
      (event: ReactPointerEvent<HTMLButtonElement>, node: CanvasNode, side: CanvasSide) => {
    ...
    useEffect(() => {
      ...
      const handlePointerUp = (event: PointerEvent) => {
        ...
        const targetNode = targetNodeId
          ? draftDocumentRef.current.nodes.find((node) => node.id === targetNodeId)
          : null;
        if (targetNode && targetNode.id !== activeDrag.fromNodeId && !isGroupNode(targetNode)) {
    ...
    }, [applyDraftChange, copySelectedNodes, deleteSelection, mode, pasteNodes, selectedNodeIds]);
    ...
    const onViewportPointerDown = useCallback(
      (event: ReactPointerEvent<HTMLDivElement>) => {
        ...
        if (
          target?.closest(
            "[data-canvas-node-id], .canvas-edge-hit, .canvas-floating-toolbar, .business-canvas-context-menu, .canvas-connection-drop-popup",
          )
        ) {
          return;
        }
    ...
    const onNodePointerDown = useCallback(
      (event: ReactPointerEvent<HTMLElement>, node: CanvasNode) => {
        ...
        setContextMenu(null);
        setEditingEdgeId(null);
        if (armedConnectionNodeId && armedConnectionNodeId !== node.id && !isGroupNode(node)) {
    ...
    const onViewportWheel = useCallback(
      (event: ReactWheelEvent<HTMLDivElement>) => {
        ...
        const nextZoom = clampZoom(
          Number(
            (viewport.zoom + (event.deltaY < 0 ? WHEEL_ZOOM_STEP : -WHEEL_ZOOM_STEP)).toFixed(2),
          ),
        );
        const rect = viewportRef.current?.getBoundingClientRect();
        if (!rect) {
      ...
      ? ({
          transform: selectedToolbarIsGroup
            ? `translate(calc(${
              ...
              }px - 100%), ${viewport.y + Math.max(0, selectedBounds.y - 46) * viewport.zoom}px)`
    ...
    const connectionDropPromptStyle = connectionDropPrompt
      ? ({
          ...
          }px, ${viewport.y + (INTERNAL_ORIGIN + connectionDropPrompt.point.y) * viewport.zoom}px)`,
        } as CSSProperties)
      : undefined;
    ...
    const canEdit =
      mode === "edit" && canEditSource && hasSource && !isSaving && !loadValidationError;
    const rootClassName = ["panel preview-panel canvas-panel business-canvas-editor", className]
      .filter(Boolean)
      .join(" ");
    const bodyClassNames = ["panel-body preview-body", bodyClassName].filter(Boolean).join(" ");
    ...
    return (
      <section className={rootClassName} data-canvas-mode={mode} data-md-block-control="true">
        <div className="panel-header">
          ...
          <div className="preview-actions">
            {showModeToggle ? (
              <div className="preview-mode-toggle" role="group" aria-label="Canvas mode">
                ...
                <CanvasIconButton
                  ...
                  disabled={
                    !hasSource || isSaving || Boolean(loadValidationError) || !canEditSource
                  }
                >
                  <CanvasEditIcon />
                </CanvasIconButton>
            ...
            {toolbarActions ? (
              <div className="canvas-toolbar-row canvas-embedded-actions">{toolbarActions}</div>
            ) : null}
              ...
              <>
                ...
                >
                  ...
                  >
                        ...
                        return (
                          ...
                          >
                            ...
                            <path
                              ...
                              markerStart={
                                edge.fromEnd === "arrow" ? "url(#canvas-arrow-start)" : undefined
                              }
                              markerEnd={
                    ...
                    {selectionRectStyle ? (
                      <div className="business-canvas-selection-rect" style={selectionRectStyle} />
                    ) : null}
                        ...
                        >
                          <div className="canvas-node-body">
                            {isEditing ? (
                              <div
                                ...
                                onInput={(event) =>
                                  updateNodeText(node.id, event.currentTarget.textContent ?? "")
                                }
                                onBlur={commitNodeEditing}
                                onKeyDown={(event) => {
                                  if (event.key === "Escape") {
                          ...
                          {CANVAS_SIDES.map((side) => (
                            <button
                              ...
                              title={`Connect ${side}`}
                              onPointerDown={(event) => startConnectionDrag(event, node, side)}
                            />
                          ))}
                          {isSelected ? (
                            <button
                ...
                <div className="business-canvas-toolbar-layer">
                          ...
                          <CanvasIconButton
                            ...
                            onClick={() =>
                              setArmedConnectionNodeId((current) =>
                                current === selectedSingleNode.id ? null : selectedSingleNode.id,
                              )
                            }
                          >
                            <CanvasConnectIcon />
                  ...
                  {mode === "edit" && selectedEdge && edgeToolbarStyle ? (
                    <CanvasFloatingToolbar style={edgeToolbarStyle}>
                      <CanvasEdgeDirectionPicker
                        ...
                        onChange={(direction) => updateEdgeDirection(selectedEdge.id, direction)}
                      />
                  ...
                  {mode === "edit" && connectionDropPrompt && connectionDropPromptStyle ? (
                    <div className="canvas-connection-drop-popup" style={connectionDropPromptStyle}>
                      <CanvasIconButton
                        label="Neue Karte"
